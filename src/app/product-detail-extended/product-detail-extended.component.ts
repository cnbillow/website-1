...
@Component({
  selector: 'app-product-detail-extended',
  templateUrl: './product-detail-extended.component.html',
  styleUrls: ['./product-detail-extended.component.sass'],
  providers: [ServicesService]
})
export class ProductDetailExtendedComponent implements OnInit {
          ...
          this.req = this._service.list().subscribe(data => {
              data.filter(item => {
                  if(item.slug == this.slug){
                      ...
                      if (this.product.bullets && this.product.bullets.length > 5){
                          let smallLi = true;
                      }
                  }
              })
